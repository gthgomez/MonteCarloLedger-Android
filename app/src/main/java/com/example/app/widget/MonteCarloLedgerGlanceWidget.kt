    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val widgetData = withContext(Dispatchers.IO) {
            runCatching {
                ...
                val bankBalanceCents = settings["bank_balance_cents"]?.toIntOrNull() ?: 0
                ...
                val mc = MonteCarloEngine(MonteCarloParams(runs = 100, includeDailyPercentiles = false)).runSimulation(forecastSeedCents.toLong(), events, today)
                val riskLabel = when {
