...
class ForecastEngineTest {
    ...
    fun forecastMath_accountsForIncomeAndBillEvents() {
        ...
        assertEquals(7_000L, ForecastEngine.calculateSafeToSpend(balance, events))
        assertEquals(1_000L, ForecastEngine.calculateDailySafeSpend(balance, events, 7))
    ...
    fun calculateSafeToSpend_allIncomeScenario() {
        val balance = 100_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Pay 1", 50_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Pay 2", 30_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 8), "Pay 3", 20_00L, "income"),
        )
        // Only income: balance never drops below starting balance
        assertEquals(100_00L, ForecastEngine.calculateSafeToSpend(balance, events))
    }
    ...
    fun calculateSafeToSpend_allExpenseScenario_goesNegative() {
        val balance = 100_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Rent", 60_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Car", 50_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 8), "Food", 40_00L, "bill"),
        )
        // Running: 10000 -> 4000 -> -1000 -> -5000, lowest = -5000
        assertEquals(-50_00L, ForecastEngine.calculateSafeToSpend(balance, events))
    }

    @Test
    fun calculateSafeToSpend_emptyEvents_returnsBalance() {
        assertEquals(500_00L, ForecastEngine.calculateSafeToSpend(500_00L, emptyList()))
    }

    @Test
    fun calculateSafeToSpend_outOfOrderEvents_sortedCorrectly() {
        val balance = 1000_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 3), "Rent", 300_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 1), "Payday", 500_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 2), "Insurance", 100_00L, "bill"),
        )
        // After sort: income day1, bill day2, bill day3
        // Running: 100000 -> 150000 -> 140000 -> 110000, lowest = 100000
        assertEquals(1000_00L, ForecastEngine.calculateSafeToSpend(balance, events))
    }
    ...
    fun calculateSafeToSpend_sameDayIncomeBeforeBill() {
        ...
        val balance = 50_00L
        val events = listOf(
            ForecastEvent(sameDay, "Rent", 100_00L, "bill"),
            ForecastEvent(sameDay, "Payday", 200_00L, "income"),
        )
    ...
    fun calculateDailySafeSpend_negativeSafeToSpend_returnsZero() {
        val balance = 10_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Big bill", 20_00L, "bill"),
        )
        assertEquals(0L, ForecastEngine.calculateDailySafeSpend(balance, events, 5))
    }

    @Test
    fun calculateDailySafeSpend_zeroDaysUntilPayday_returnsFullSafeToSpend() {
        val balance = 100_00L
        assertEquals(100_00L, ForecastEngine.calculateDailySafeSpend(balance, emptyList(), 0))
    }
    ...
    fun calculateDailySafeSpend_dividesByDaysUntilPayday() {
        val balance = 100_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Income", 50_00L, "income"),
        )
        // safeToSpend = 100_00, daysUntilPayday = 5
        assertEquals(20_00L, ForecastEngine.calculateDailySafeSpend(balance, events, 5))
    }
    ...
    fun calculateIncomeContribution_noIncome_returnsZero() {
        val balance = 100_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Bill1", 5_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Bill2", 5_00L, "bill"),
        )
        // Both passes yield safeToSpend=0, so contribution = 0
        assertEquals(0L, ForecastEngine.calculateIncomeContribution(balance, events))
    }
    ...
    fun calculateIncomeContribution_negativeProjectedBalance_handledCorrectly() {
        val balance = 10_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Income", 5_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Big bill", 20_00L, "bill"),
        )
    ...
    fun calculateIncomeContribution_incomeImprovesSafetyMargin() {
        ...
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Income", 50_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Expense", 60_00L, "bill"),
        )
        assertEquals(50_00L, ForecastEngine.calculateIncomeContribution(balance, events))
    }

    @Test
    fun calculateIncomeContribution_emptyEvents_returnsZero() {
        // No events: both paths stay at starting balance, contribution = 0
        assertEquals(0L, ForecastEngine.calculateIncomeContribution(500_00L, emptyList()))
    }
    ...
    fun calculateIncomeContribution_incomeOnly_noBillsMeansNoImprovement() {
        ...
        val balance = 200_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 3), "Paycheck 1", 100_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 8), "Paycheck 2", 150_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 12), "Bonus", 50_00L, "income"),
        )
        assertEquals(0L, ForecastEngine.calculateIncomeContribution(balance, events))
    }
    ...
    fun calculateIncomeContribution_multiIncomeMultiBill_computesBufferImprovement() {
        ...
        val balance = 200_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 3), "Bill A", 50_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Paycheck 1", 150_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 8), "Bill B", 180_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 10), "Paycheck 2", 100_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 12), "Bill C", 120_00L, "bill"),
        )
        assertEquals(100_00L, ForecastEngine.calculateIncomeContribution(balance, events))
    }
    ...
    fun calculateIncomeContribution_incomeLiftsBufferAboveBillsOnlyFloor() {
        ...
        val balance = 500_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Payday", 200_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 3), "Rent", 100_00L, "bill"),
            ForecastEvent(LocalDate.of(2026, 1, 4), "Freelance", 300_00L, "income"),
            ForecastEvent(LocalDate.of(2026, 1, 5), "Utilities", 150_00L, "bill"),
        )
        assertEquals(250_00L, ForecastEngine.calculateIncomeContribution(balance, events))
    }
    ...
    fun buildBalanceForecast_sequence() {
        ...
        val events = listOf(
            ...
            ForecastEvent(LocalDate.of(2026, 1, 10), "Car", 100_00L, "bill"),
        )
        val forecast = ForecastEngine.buildBalanceForecast(balance, events)
        assertEquals(3, forecast.size)
        assertEquals(LocalDate.of(2026, 1, 2), forecast[0].date)
        assertEquals(1500_00L, forecast[0].balanceCents)
        assertEquals(LocalDate.of(2026, 1, 5), forecast[1].date)
        assertEquals(1200_00L, forecast[1].balanceCents)
        assertEquals(LocalDate.of(2026, 1, 10), forecast[2].date)
        assertEquals(1100_00L, forecast[2].balanceCents)
    }

    @Test
    fun buildBalanceForecast_emptyEvents_returnsEmptyList() {
        val forecast = ForecastEngine.buildBalanceForecast(500_00L, emptyList())
    ...
    fun buildBalanceForecast_incomeAndExpenseOnSameDay_appliesIncomeFirst() {
        ...
        val balance = 100_00L
        val events = listOf(
            ForecastEvent(sameDay, "Rent", 200_00L, "bill"),
            ForecastEvent(sameDay, "Payday", 300_00L, "income"),
        )
        // Sort: income first → balance: 100+300=400, then 400-200=200
        val forecast = ForecastEngine.buildBalanceForecast(balance, events)
        assertEquals(2, forecast.size)
        assertEquals(400_00L, forecast[0].balanceCents)
        assertEquals(200_00L, forecast[1].balanceCents)
    }
    ...
    fun calculateForecastSummary_goesNegativeAndRecovers() {
        val balance = 500_00L
        val events = listOf(
            ForecastEvent(LocalDate.of(2026, 1, 2), "Big bill", 600_00L, "bill"),
